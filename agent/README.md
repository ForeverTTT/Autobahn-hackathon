...
└─────────────────────────────────────────────────────────────────────────┘
                    ...
                    ▼                               ▼
┌─────────────────────────────┐     ┌─────────────────────────────┐
│      ForecastAgent          │     │      ContextAgent           │
│      (预测Agent)             │     │      (上下文Agent)           │
│                             │     │                             │
│  读取同一时间段的预测数据：    │     │  查询同一时间段的外部因素：    │
│  - kfz_h (P10/P50/P90)      │     │  - 天气、气温                 │
│  - v_kfz 车速               │     │  - 假期、学校假期             │
│  - sv_h 重车流量            │     │  - 历史交通参考               │
│  - 拥堵分数计算              │     │  - 季节性因素                 │
└─────────────────────────────┘     └─────────────────────────────┘
...
└─────────────────────────────────────────┘
    ...
    ▼
┌─────────────┐
│ SearchAgent │  实时搜索（施工、天气预报、活动）
└─────────────┘
    │
    ▼
┌─────────────┐
│GenerationAgent│  生成自然语言 + 方案对比 + 个性化建议
└─────────────┘
    │
    ▼
用户收到回复
...
1. **ForecastAgent 和 ContextAgent 查询同一时间段**
   - 确保预测数据和外部因素对应
   - 例如：都查询 2026-07-25 的 08:00-18:00

2. **SearchAgent 获取实时信息**
   - 天气预报只能获取未来 15 天
   - 施工公告需要实时 API

3. **GenerationAgent 生成可对比方案**
   - 至少提供 3 个不同出发时间的方案
   - 包含压力指数便于用户选择
