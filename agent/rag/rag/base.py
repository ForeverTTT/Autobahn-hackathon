...
# allows to use different .env file for each rag instance
...
class OllamaServerInfos:
    def __init__(self, name=None, tag=None):
        self._rag_name = name or os.getenv(
            "OLLAMA_EMULATING_MODEL_NAME", DEFAULT_OLLAMA_MODEL_NAME
        )
        self._rag_tag = tag or os.getenv(
            "OLLAMA_EMULATING_MODEL_TAG", DEFAULT_OLLAMA_MODEL_TAG
        )
        self.RAG_SIZE = DEFAULT_OLLAMA_MODEL_SIZE
        self.RAG_CREATED_AT = DEFAULT_OLLAMA_CREATED_AT
        self.RAG_DIGEST = DEFAULT_OLLAMA_DIGEST
    ...
    @RAG_NAME.setter
    def RAG_NAME(self, value):
        self._rag_name = value

    @property
    def RAG_TAG(self):
        return self._rag_tag

    @RAG_TAG.setter
    def RAG_TAG(self, value):
        self._rag_tag = value

    @property
    def RAG_MODEL(self):
        return f"{self._rag_name}:{self._rag_tag}"
...
@dataclass
class QueryParam:
    """Configuration parameters for query execution in RAG."""
    ...
    """Number of text chunks to retrieve from vector search. If None, defaults to top_k value."""
...
class QueryResult:
    ...
    def metadata(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Query metadata including keywords, etc.
        """
        if self.raw_data:
