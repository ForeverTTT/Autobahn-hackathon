...
async def kg_query(
    ...
    cached_result = await handle_cache(
        hashing_kv, args_hash, user_query, cache_type="query"
    )
...
async def extract_keywords_only(
    ...
    cached_result = await handle_cache(
        hashing_kv, args_hash, text, cache_type="keywords"
    )
    if cached_result is not None:
        cached_response, _ = cached_result  # Extract content, ignore timestamp
        try:
...
async def _perform_kg_search(
    ...
    """
    Pure search logic that retrieves raw entities and relations (hybrid mode).
    ...
    # Hybrid mode: both local and global paths
    ...
    logger.info(
        f"Raw search results: {len(final_entities)} entities, {len(final_relations)} relations"
    )
...
async def _merge_all_chunks(
    ...
    """
    Merge chunks from entity_chunks + relation_chunks (hybrid mode).
    """
    ...
    # Round-robin merge chunks from entity and relation sources with deduplication
    merged_chunks = []
    seen_chunk_ids = set()
