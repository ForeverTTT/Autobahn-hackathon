...
from rag.exceptions import PipelineNotInitializedError
...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    # Get the current logger level from the rag logger
    try:
        from rag.utils import logger

        current_level = logger.getEffectiveLevel()
    except ImportError:
        # Fallback if rag.utils is not available
        ...
        pipeline_namespace.update(
            {
                ...
                "history_messages": history_messages,  # Use shared list object
            }
        )
