"""
生成带拥堵评分的 CSV 文件
读取 forecast_2026_2029.csv，计算每小时的 congestion_score 和 level
"""
import pandas as pd
import numpy as np
...
FORECAST_FILE = DATA_DIR / "forecast_2026_2029.csv"
...
OUTPUT_FILE = DATA_DIR / "scored_traffic_2026_2029.csv"
...
def load_external_factors():
    ...
    weather_df = weather_df.iloc[1:]  # 跳过中文表头
...
def calculate_congestion_score(row, holiday_lookup, weather_lookup, event_lookup, construction_lookup):
    """计算单行的拥堵分数"""

    # 交通数据
    kfz_h = float(row['kfz_h_p50'])
    sv_h = float(row['sv_h_pred'])
    ...
    return round(total_score, 1), level, is_weekend, is_holiday, is_school_holiday, is_peak_hour, weather_impact > 0, has_event, has_construction


def main():
    print("=" * 60)
    print("生成带拥堵评分的 CSV 文件")
    print("=" * 60)
    ...
    for i, (idx, row) in enumerate(forecast_df.iterrows()):
        ...
        score, level, is_weekend, is_holiday, is_school_holiday, is_peak, has_weather, has_event, has_construction = \
            calculate_congestion_score(row, holiday_lookup, weather_lookup, event_lookup, construction_lookup)
